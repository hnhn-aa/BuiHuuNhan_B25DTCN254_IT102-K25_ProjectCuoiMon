...
int addAccount(Account accounts[], int *count){
	...
	while (1) {
        ...
        if (strlen(acc.phone) == 0) {
            printf("Error: Phone number cannot be blank!!!\n");
            continue;
        }

        int valid = 1;
        for (int i = 0; acc.phone[i] != '\0'; i++) {
            if (!isdigit((unsigned char)acc.phone[i])) {
                valid = 0;
                break;
            }
        }
...
void updateAccount(Account accounts[], int count){
	...
	do{
		...
		if(strlen(newPhone)==0){
			printf("Error: Phone cannot be blank!!!\n");
		}	
	}while(strlen(newPhone)==0);
...
void welcomeScreen() {
    ...
    char title1[] = "	 WELCOME TO	   ";
    char title2[] = "MINI BANKING SYSTEM";
    ...
    printf("\nPress Enter To Log Into The System...");
    getchar();
}
...
void goodbyeScreen() {
    ...
    char msg2[] = "Wishing you a great day ahead!";
